"""ResNet-18 classifier and the approved frozen-BatchNorm policy for Task 3."""
...
def freeze_batchnorm_statistics(model: nn.Module) -> None:
    """Freeze running buffers while leaving gamma and beta trainable."""
...
def assert_batchnorm_unchanged(
    ...
    changed = [
        name for name in expected if not torch.equal(expected[name], actual[name])
    ]
    if changed:
